...
public class SystemTestingPresenter extends ActivityLogger {
    ...
    @FXML
    private TextArea txtAreaFacts;
    @FXML
    private ImageView imgView;
    @FXML
    private JFXButton btnChooseFile;
    @FXML
    private JFXButton btnClassify;
    @FXML
    ...
    @FXML
    private JFXTextField txtAlexPredicted;
    @FXML
    private JFXTextField txtGooglePredicted;
    @FXML
    private JFXTextField txtResNetPredicted;
    @FXML
    private JFXTextField txtFruit114Predicted;
    @FXML
    private JFXTextField txtAlexAccuracy;
    @FXML
    private JFXTextField txtGoogleAccuracy;
    @FXML
    private JFXTextField txtResNetAccuracy;
    @FXML
    private JFXTextField txtFruit114Accuracy;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        btnPrint.setOnAction(event -> {
            if (imgView.getImage() == null
                    || txtAreaFacts.getText() == null || txtAreaFacts.getText().isEmpty()) {
                PresenterUtils.INSTANCE.displayError("Please select image and classify.");
                return;
            }
            try {
                Map<String, String> map = new HashMap<>();
                map.put("facts", txtAreaFacts.getText());
                map.put("txtAlexPredicted", txtAlexPredicted.getText());
                map.put("txtGooglePredicted", txtGooglePredicted.getText());
                map.put("txtResNetPredicted", txtResNetPredicted.getText());
                map.put("txtFruit114Predicted", txtFruit114Predicted.getText());
                map.put("txtAlexAccuracy", txtAlexAccuracy.getText());
                map.put("txtGoogleAccuracy", txtGoogleAccuracy.getText());
                map.put("txtResNetAccuracy", txtResNetAccuracy.getText());
                map.put("txtFruit114Accuracy", txtFruit114Accuracy.getText());
                ...
                if (bufferedImage != null) {
                    ...
                    ImageIO.write(bufferedImage, "jpg", new File("./images/image"));
                    map.put("imageUrl", "./images/image");
                }
                PrintUtil.print(new File("./reports/system_testing.jasper"), map);
            } catch (JRException | IOException e) {
                PresenterUtils.INSTANCE.displayError("Error Printing Report");
                e.printStackTrace();
            }
        });
